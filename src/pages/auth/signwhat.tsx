...
export default function SignWhat() {
    const router = useRouter();

    const handleSelectRole = (role: 'individu' | 'komunitas') => {
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-[url('/bege.png')] bg-center bg-cover">
            <form className="flex flex-col items-center justify-center bg-[#282C34]/95 w-2/5 h-96 rounded-[16px] backdrop-blur-sm gap-3">
                ...
                <div className="flex flex-col w-10/12 gap-3 capitalize">
                    <button type="button" onClick={() => handleSelectRole('komunitas')}
                        className="mt-2 p-2 h-20 rounded-[12px] bg-[#008080] text-white font-bold capitalize hover:bg-[#008080]/40">Komunitas
                    </button>
                    <button type="button" onClick={() => handleSelectRole('individu')}
                        className="mt-2 p-2 h-20 rounded-[12px] bg-[#008080] text-white font-bold capitalize hover:bg-[#008080]/40">Individu
                    </button>
                </div>
            </form>
        </div>
